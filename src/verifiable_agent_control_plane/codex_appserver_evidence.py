...
def evaluate_appserver_notifications(events: Iterable[dict[str, Any]]) -> AppServerEvidence:
    ...
    for event in events:
        ...
        record = {
            ...
            "status": item.get("status"), "exit_code": item.get("exitCode"), "source": item.get("source", "agent"),
        }
    ...
    if started is not None and any(started[key] != completed[key] for key in ("thread_id", "turn_id", "call_id", "process_id")): return AppServerEvidence(AppServerVerdict.MISMATCH)
    ...
    return AppServerEvidence(
        ...
        unified_exec_observed=source in {"unifiedExecStartup", "unifiedExecInteraction"},
