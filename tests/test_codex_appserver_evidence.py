...
class CodexAppServerEvidenceTests(unittest.TestCase):
    ...
    def test_identical_completed_replay_is_idempotent(self):
        event = completed()
        self.assertEqual(evaluate_appserver_notifications([started(), event, event]).verdict, AppServerVerdict.COMMAND_TERMINAL)

    def test_divergent_or_distinct_second_completion_is_mismatch(self):
        self.assertEqual(evaluate_appserver_notifications([started(), completed(), completed(call="call2", process="p2")]).verdict, AppServerVerdict.MISMATCH)
        self.assertEqual(evaluate_appserver_notifications([started(), completed(), completed(exit_code=1, status="failed")]).verdict, AppServerVerdict.MISMATCH)

    def test_terminal_status_exit_code_mismatch_is_mismatch(self):
        self.assertEqual(evaluate_appserver_notifications([started(), completed(status="failed", exit_code=0)]).verdict, AppServerVerdict.MISMATCH)
        self.assertEqual(evaluate_appserver_notifications([started(), completed(status="completed", exit_code=7)]).verdict, AppServerVerdict.MISMATCH)

    def test_command_terminal_without_turn_completion_is_distinct(self):
        self.assertEqual(evaluate_appserver_notifications([started(), completed()]).verdict, AppServerVerdict.COMMAND_TERMINAL)

    def test_unknown_source_or_missing_process_is_unknown(self):
        self.assertEqual(evaluate_appserver_notifications([started(source="mystery"), completed(source="mystery")]).verdict, AppServerVerdict.UNKNOWN)
        ...
        self.assertEqual(evaluate_appserver_notifications([started(), *missing]).verdict, AppServerVerdict.UNKNOWN)
    ...
    def test_turn_completed_for_different_turn_is_mismatch(self):
        events = [started(), completed(), {"method": "turn/completed", "params": {"threadId": "th1", "turnId": "tu2"}}]
